...
void main() {
  ...
  group('BorderoInputItem.toJson()', () {
    test('inclui amount_cents e due_date', () {
      final json = item.toJson();
      expect(json['amount_cents'], equals(100000));
      expect(json['due_date'], equals('2026-07-24'));
    });

    test('não inclui awaiting_days nem status', () {
      final json = item.toJson();
      expect(json.containsKey('awaiting_days'), isFalse);
      expect(json.containsKey('status'), isFalse);
    });
  });

  group('BorderoInput.toSaveJson()', () {
    test('inclui awaiting_days no nível do borderô', () {
      ...
      expect(json['awaiting_days'], equals(3));
    });

    test('itens novos (sem ID) são enviados em "receivables"', () {
      final input = BorderoInput(
        changeDate: DateTime(2026, 5, 24),
        monthlyRatePercent: 2.0,
        awaitingDays: 2,
      ...
      expect(json['receivables'][0].containsKey('status'), isFalse);
      expect(json['receivables'][0].containsKey('awaiting_days'), isFalse);
    });
